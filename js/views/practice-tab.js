...
export function renderPracticeTab(App, el) {
  ...
  const labRow = labOn
    ? `<button class="inst-row-item" id="prLab">
        <span class="grow">${en ? 'HTTP Lab (simulated server)' : 'HTTP Lab (simulated server)'}</span>
  ...
  el.innerHTML = `<div class="screen inst-screen">
    <div class="lt-head">
      <div class="kicker">${App.t('nav_practice')}</div>
      <h1>${en ? 'Study & labs' : 'Study aur labs'}</h1>
    ...
    <div class="inst-card accent-rule">
      <div class="kicker">${en ? 'Study hub' : 'Study markaz'}</div>
    ...
    <div class="slabel mt16">${en ? 'Markets loop' : 'Markets cycle'}</div>
    <div class="inst-list">
      <button class="inst-row-item" id="prReview" ${reviewAvailable() || reviewN ? '' : 'disabled'}>
        <span class="grow">${en ? 'Daily review' : 'Rozana review'}${reviewN ? ` · ${reviewN}` : ''}${missDue ? (en ? ' (incl. sim fails)' : ' (sim fails)') : ''}</span>
        <span class="mono">→</span>
      </button>
      <button class="inst-row-item" id="prCharts">
        <span class="grow">${en ? 'Charts hub' : 'Charts markaz'}</span>
        <span class="mono">→</span>
      </button>
    ...
    <div class="inst-list">
      ...
      <button class="inst-row-item" id="prDrills">
        <span class="grow">${en ? 'Market drills' : 'Bazaar drills'}</span>
        <span class="mono">→</span>
      </button>
      <button class="inst-row-item" id="prSim">
        <span class="grow">${en ? 'Practice ledger (paper lab)' : 'Practice ledger (paper)'}</span>
        <span class="mono">→</span>
      </button>
    </div>
  ...
  el.innerHTML = `<div class="screen inst-screen">
    ...
    ${!session.show
      ...
      : `<div class="srs-grades">
          <button class="btn ghost" data-g="again">${en ? 'Again' : 'Dubara'}</button>
          <button class="btn secondary" data-g="hard">${en ? 'Hard' : 'Mushkil'}</button>
          <button class="btn secondary" data-g="good">${en ? 'Good' : 'Theek'}</button>
          <button class="btn accent" data-g="easy">${en ? 'Easy' : 'Asaan'}</button>
        </div>`}
